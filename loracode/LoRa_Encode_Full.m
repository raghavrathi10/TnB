% NOTE: just need zlecc_hdr_crc, zlecc_16_bit_crc, zlecc_white
...
function [packet] = LoRa_Encode_Full(message,SF,CR_w_zz_info)
...
% zz add bgn
CR = mod(CR_w_zz_info-1,4) + 1;
zz_flag = (CR_w_zz_info > CR);
% zz add 
...
% zz add end
n_packet = 8 + max([ceil((8*(length(message_dbl)) - 4.*SF + 28 + 16.*CRC_pld - 20.*imp)/(4.*(SF - 2.*opt))).*(CR + 4) 0]) ;
...
n_pad = n_pld - length(message_dbl) - CRC_pld.*2 ;
%% Create payload message
CRC_dbl = CRC_pld.*[1 1] ; % CRC is not working atm
pad_dbl = zeros(1,n_pad + N_pld - 1) ; % padding

pld_dbl = [message_dbl CRC_dbl pad_dbl] ; % LoRa payload
len_b4_crc = length(pld_dbl)-length(CRC_dbl)-length(pad_dbl);
CRC_dbl = zlecc_16_bit_crc(pld_dbl(1:len_b4_crc));
pld_dbl(1+length(message_dbl):2+length(message_dbl)) = CRC_dbl(:);
pld_dbl(1:len_b4_crc) = zlecc_white(pld_dbl(1:len_b4_crc));
...
pld_enc_CR1 = LoRa_encode_parity_CR1(pld_swp);
pld_enc_CR1 = pld_enc_CR1(1 : n_wht);
%% Payload Whiten
% pld_wht = LoRa_encode_white(pld_enc,CR,0) ;
pld_wht = pld_enc;
pld_wht_CR1 = pld_enc_CR1;
%% Header Encoding
packet_hdr = [(length(message_dbl)) CRC_pld*16+(CR==1)*32+(CR==2).*64+(CR==3).*96+(CR==4)*128 224] ;
% zz add bgn
packet_hdr_crcbits = zlecc_hdr_crc(packet_hdr);
packet_hdr(2) = bitand(packet_hdr(2),254);
packet_hdr(2) = packet_hdr(2) + packet_hdr_crcbits(5);
packet_hdr(3) = bi2de(packet_hdr_crcbits(1:4))*16;
% zz add end
packet_hdr_enc_tmp = LoRa_encode_hamming(packet_hdr,4) ;
packet_hdr_enc = [packet_hdr_enc_tmp(1:5) pld_wht(1:orig_N_pld-1)] ; % zz changed
%% Packet Creation
if CR > 1
    packet_pld = pld_wht(N_pld : end) ;
else
    packet_pld = pld_wht_CR1(N_pld : end) ;
end
...
function [encoded] = LoRa_encode_parity_CR1(symbols)
    Ctr = 1 ;
    for ctr = 1 : length(symbols)
        s0 = bitand(floor(bitsra(symbols(ctr),4)),hex2dec('FF')) ;
        s1 = bitand(floor(bitsra(symbols(ctr),0)),hex2dec('FF')) ;
        encoded(Ctr+0) = selectbits_encode_CR1(s0) ;
        encoded(Ctr+1) = selectbits_encode_CR1(s1) ;
        Ctr = Ctr + 2 ;
    end
end
...
% if 1 % CR > 2 && CR <= 4 % detection and correction % ZZ: changed, 
% 
...
function [symbol_rot] = selectbits_encode_CR1(symbol)
% selectbits_encode concat zeros (from 8-bit to 4-bit)
%
%   in:  symbols            symbol sequence
%
...
symbol_binary = de2bi(symbol,8);
parirtyval = mod(sum(symbol_binary(1:4)),2);
symbol_binary_rot = [0 symbol_binary(1) symbol_binary(2) symbol_binary(3) parirtyval symbol_binary(4) 0 0] ;
symbol_rot = bi2de(symbol_binary_rot) ;
end
